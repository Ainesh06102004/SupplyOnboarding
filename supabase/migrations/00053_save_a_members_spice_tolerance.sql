-- ============================================================================
-- 00053_save_a_members_spice_tolerance
--
-- 00052 gave a member a spice tolerance; this lets the profile form save it.
--
-- save_household_member() names every field it writes, on purpose: a function
-- that wrote whatever it was handed would let a form set a column no form is
-- meant to touch. So a new field is a new line here, in both the insert and
-- the update, and the update keeps the partial-field rule — a form that does
-- not send spice_tolerance leaves it exactly as it was.
--
-- Nothing else about the function changes. It is replaced whole rather than
-- patched, so the definition in the repository is the definition in the
-- database.
-- ============================================================================
...
BEGIN
  ...
  IF v_id IS NULL THEN
    INSERT INTO public.household_member (
      ...
      energy_goal, eating_pattern, age_years, weight_kg, height_cm, appetite, spice_tolerance, meals_from_home,
